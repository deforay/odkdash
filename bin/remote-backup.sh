...
# Sets up automatic off-machine backups of an ODK Dashboard installation.
#
...
#      the ODK Central accounts the dashboard pulls submissions with
#
...
#
# Answers are saved to /etc/odkdash/backup.conf, so re-running this script
# is a matter of pressing Enter through the prompts. The backup runner installed
# at /usr/local/bin/odkdash-backup.sh reads that file, so nothing is
...
CONF_DIR="/etc/odkdash"
CONF_FILE="${CONF_DIR}/backup.conf"
RUNNER="/usr/local/bin/odkdash-backup.sh"
INSTANCE_UUID_FILE="${CONF_DIR}/instance-uuid"
SSH_KEY="/root/.ssh/id_ed25519_odkdash"
MOUNT_POINT="/mnt/odkdash-backup"
SMB_CRED_FILE="${CONF_DIR}/smb-backup.cred"
STAGING_DIR="/var/odkdash-backup/staging"
...
print header "ODK Dashboard backup setup"

# --- instance name ------------------------------------------------------------

ask INSTANCE_NAME "Name for this dashboard (country or site)" "${INSTANCE_NAME:-$(hostname -s 2>/dev/null || echo odkdash)}"
...
# The same test bin/upgrade.sh uses, and for the same reason. The composer.json
# name is the marker. config/autoload/global.php is deliberately not required,
# because it is gitignored and an instance can be sitting there without one. The
# database step below says so rather than refusing the whole path here.
looks_like_odkdash() {
  [ -f "$1/composer.json" ] && [ -f "$1/public/index.php" ] &&
...
if [ -z "$APP_PATH" ]; then
  for candidate in /var/www/odkdash /var/www/odk-dashboard; do
    if looks_like_odkdash "$candidate"; then APP_PATH="$candidate"; break; fi
  done
fi
if [ -z "$APP_PATH" ]; then
...
while true; do
  ask APP_PATH "ODK Dashboard folder path" "${APP_PATH:-/var/www/odkdash}"
  ...
  if ! looks_like_odkdash "$APP_PATH"; then
    print warning "'$APP_PATH' does not look like an ODK Dashboard installation (composer.json is not deforay/odkdash)."
    APP_PATH=""
    continue
  fi
  break
...
configure_ssh() {
  ...
  if [ ! -f "$SSH_KEY" ]; then
    ...
    ssh-keygen -t ed25519 -C "odkdash-backup-${SANITIZED_NAME}" -N "" -f "$SSH_KEY" >/dev/null
  fi
  ...
  DEST_BASE="${remote_home}/odkdash-backups"
...
configure_smb() {
  ...
  print info "On the Windows machine: share a folder (for example C:\\ODKDash-Backups)"
  ...
  while true; do
    ...
    ask SMB_SHARE "Name of the shared folder (for example ODKDash-Backups)" "${SMB_SHARE:-}"
    ask SMB_USER  "Windows username" "${SMB_USER:-}"
    ask_secret smb_pass "Windows password for ${SMB_USER}"

    if printf '%s' "$SMB_SHARE" | grep -q ' '; then
      print warning "The share name contains a space. Re-share the folder using a name without spaces, such as ODKDash-Backups."
      SMB_SHARE=""
      continue
    fi
    ...
    if ! ( : > "${MOUNT_POINT}/.odkdash-writetest" && rm -f "${MOUNT_POINT}/.odkdash-writetest" ); then
  ...
  DEST_BASE="${MOUNT_POINT}/odkdash-backups"
  DEST_DIR="${DEST_BASE}/${DEST_FOLDER}"
}
...
configure_local() {
  ...
  while true; do
    ...
    if ! ( : > "${LOCAL_ROOT}/.odkdash-writetest" && rm -f "${LOCAL_ROOT}/.odkdash-writetest" ); then
  ...
  DEST_BASE="${LOCAL_ROOT}/odkdash-backups"
  DEST_DIR="${DEST_BASE}/${DEST_FOLDER}"
}
...
umask 077
cat > "$CONF_FILE" <<CONF
# ODK Dashboard backup configuration. Written by bin/remote-backup.sh.
...
#!/bin/bash
# ODK Dashboard backup runner. Installed by bin/remote-backup.sh. It reads its
# settings from /etc/odkdash/backup.conf. Safe to run by hand at any time.
#
...
CONF_FILE="/etc/odkdash/backup.conf"
STATE_DIR="/var/lib/odkdash"
STATUS_JSON="${STATE_DIR}/backup-status.json"
STATUS_ENV="${STATE_DIR}/backup-status.env"
LOGFILE="/var/log/odkdash-backup.log"
LOCKFILE="/var/lock/odkdash-backup.lock"
RESTORE_ROOT="/var/odkdash-backup/restore"
SAFETY_ROOT="/var/odkdash-backup/before-restore"

usage() {
  cat <<USAGE
ODK Dashboard backup runner.

  odkdash-backup.sh                 Run a backup now
  odkdash-backup.sh --status        Show when the last backup ran and whether it worked
  odkdash-backup.sh --test          Check the connection and the database, changing nothing
  odkdash-backup.sh --list          List the backups held at the destination
  odkdash-backup.sh --restore [WHICH] [--yes]
                                          Restore the database and config/autoload.
                                          WHICH is a folder name from --list, or "latest"
  odkdash-backup.sh --disable       Stop the scheduled backups
  odkdash-backup.sh --enable        Start the scheduled backups again
  odkdash-backup.sh --help          Show this message
USAGE
}
...
: "${SSH_USER:=}"; : "${SSH_HOST:=}"; : "${SSH_PORT:=22}"; : "${SSH_KEY:=/root/.ssh/id_ed25519_odkdash}"
: "${SMB_HOST:=}"; : "${SMB_SHARE:=}"; : "${SMB_VERS:=3.0}"; : "${MOUNT_POINT:=/mnt/odkdash-backup}"
: "${LOCAL_ROOT:=}"; : "${STAGING_DIR:=/var/odkdash-backup/staging}"
: "${RETENTION:=14}"; : "${LOCAL_KEEP:=2}"; : "${VERIFY_CHECKSUMS:=yes}"
: "${SCHEDULE_CRON:=0 */12 * * *}"; : "${BACKUP_ENCRYPT_PASSWORD:=}"

CRON_MARKER="/usr/local/bin/odkdash-backup.sh"
...
case "$ACTION" in
  disable)
    if crontab -l 2>/dev/null | grep -q "$CRON_MARKER"; then
      ...
      echo "Scheduled backups stopped. Run 'odkdash-backup.sh --enable' to start them again."
    else
...
mysql_defaults_file() {
  local f
  f="$(mktemp /tmp/odkdash-my.XXXXXX)"
  chmod 600 "$f"
...
if [ "$ACTION" = "list" ]; then
  ...
  echo
  print info "Restore the newest one with: odkdash-backup.sh --restore latest"
  exit 0
fi
...
restore_from() {
  ...
  # The merged config cache is written to data/cache by Laminas. Where a
  # deployment has turned it on, a stale copy holds the credentials from before
  # the restore. Drop it and let the next request rebuild it.
  rm -f "${APP_PATH}"/data/cache/*.php 2>/dev/null || true

  print success "Restored the backup taken at ${run}"
  print info    "Fetched copy   : ${local_copy}"
  print info    "Previous state : ${safety}"
  print info    "Check the dashboard in a browser, then run: odkdash-backup.sh --test"
}
...
cat > /etc/logrotate.d/odkdash-backup <<'LOGROTATE'
/var/log/odkdash-backup.log {
    weekly
    rotate 8
    missingok
    notifempty
...
( crontab -l 2>/dev/null | grep -v "odkdash-backup.sh" || true ) | crontab -
