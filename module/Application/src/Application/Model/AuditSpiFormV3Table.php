 ...
 * @author amit
 */
class AuditSpiFormV3Table extends AbstractTableGateway {

    protected $table = 'audit_spi_form_v_3';
    ...
    public function fetchAllDetails($parameters)
    {
        ...
        $spiV3Db = new SpiFormVer3Table($this->adapter);

        $columnsSql = "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE  table_name = 'audit_spi_form_v_3' order by ordinal_position";
        $response['auditColumns'] = $dbAdapter->query($columnsSql, $dbAdapter::QUERY_MODE_EXECUTE)->toArray();

        $columnsSql1 = "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE  table_name = 'spi_form_v_3' order by ordinal_position";
        ...
        $response['currentRecord'] = $spiV3Db->fetchV3DetailsByMetaInstanceId($metaInstance);
        if(isset($metaInstance) && $metaInstance!=''){
                $sQuery = $sql->select()->from(array('a' => 'audit_spi_form_v_3'))
